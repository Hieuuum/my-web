...
- Macar et al.[^1] introduced a new metric called chain-of-thought (CoT) resilience, which measures how persistent a sentence is in a CoT. Inspired by the concept, I designed a new metric called thought reappearance, wanted to test whether resilience is linearly encoded in the model's activations. If it does, we can just run a  forward pass through a linear probe to predict this metric.
