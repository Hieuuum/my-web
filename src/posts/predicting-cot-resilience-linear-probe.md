...
**I didn't run the baseline.** The cleanest test of the surface-feature worry is a probe trained on only word counts and sentence length, no activations. If a bag-of-words baseline matches 75%, then the probe may have been reading lexical features instead of the model's activations.
