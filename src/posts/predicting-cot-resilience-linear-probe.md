...
- Macar et al.[^1] introduced a new metric called chain-of-thought (CoT) resilience. It measures how persistent a sentence is in a CoT by intervening at different positions, changing the model's activations, which makes it unsuitable for  I designed a new metric called thought reappearance, which aims to measure the same concept wanted to test whether resilience is linearly encoded in the model's activations. If it does, we can just run a  forward pass through a linear probe to predict this metric.
