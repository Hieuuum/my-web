...
- For my next steps, I will run a bag-of-words baseline to see if the linear probe is detecting surface features or not, and examine the model's reasoning after steering it along the learned resilience direction to verify the direction is causal, not just correlated.
...
Thought Branches[^1] measures how persistent a CoT is using thought resilience. The idea is to interpret a reasoning model not as producing one CoT, but as defining a distribution over many possible ones. To measure how committed the model is to a given sentence S_i, you truncate the chain just before that sentence, let the model continue from that prefix, and up the count by 1. If a semantically similar sentence comes back, you remove the CoT starting from the sentence with the highest semantic similarity with S_i, and resample again. Repeat the process until there is no sentence with a similar context with S_i, and the resilience score is the number of times you repeated the process. A sentence that keeps returning is resilient, while one that rarely returns is fragile.

I wanted to know if thought resilience is linearly encoded in a model's residual activations. However, Macar et al.'s resilience metric[^1] is dependent not only on the model's activations at the end of the original sentence, but also on those in the intervening positions in later sentences. This makes it hard for linear probes to reliably predict resilience based on the original sentence's activations alone.

A skeptic might say, for each iteration, why don't we collect the activations of all sentences with similar semantic meaning to the original one and predict how many iterations do we need until the thought disappears entirely from the CoT? For example, one resilience run on sentence A produces a sequence of intervention points (A at position i, then its reappearance A' at position j, then A'' at position k, then it dies after 4 steps). Instead of getting one label from that whole run, you want to harvest four labeled examples — A→4, A'→3, A''→2, A'''→1 — each paired with the activations at its own position. This gives us more training data per run and makes it easier to predict resilience, right? This unfortunately introduces another bias. Since each intervention deals with CoT in a later position than in the previous one, increasing the 
