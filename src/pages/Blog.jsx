  ...
  return (
    <div className="max-w-3xl mx-auto px-6 pt-20 pb-16">
      ...
      <a
        ...
        className="inline-block mb-12 text-sm text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-100 transition-colors"
      >
        Also on Substack →
      </a>

      {posts.length === 0 ? (
        <p className="text-slate-400 dark:text-zinc-500">No posts yet.</p>
      ) : (
            ...
            <li key={post.slug} className="py-7">
              <Link to={`/blog/${post.slug}`} className="group block">
                <div className="flex items-baseline justify-between gap-4 mb-1">
                  <h2 className="text-lg font-medium text-slate-900 dark:text-zinc-100 group-hover:text-slate-600 dark:group-hover:text-zinc-400 transition-colors">
                    {post.data.title}
                  </h2>
